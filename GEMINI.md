...
이 프로젝트는 Android 개발자가 모바일 ↔ 서버 통신, 인증, 예외처리, 파일 업로드/다운로드, 배포를 학습하기 위한 개인 프로젝트이다.

### Gemini의 책임
- 실제로 실행되고, 프론트엔드에서 호출 가능한 서버를 만든다.
- 과도한 추상화나 엔터프라이즈급 설계보다 MVP 관통을 우선한다.
- Android 클라이언트가 명확하게 예외처리할 수 있도록 API를 설계한다.
- 구현 결과는 항상 복사해서 바로 실행 가능한 전체 코드로 제공한다.
- 매 응답마다 commit으로 작성할 만한 comment를 제시한다.
